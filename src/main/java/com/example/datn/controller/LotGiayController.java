...
public class LotGiayController {
    ...
    public ResponseEntity<LotGiayDto> createLotGiay(@RequestBody LotGiayDto lotGiayDto) {
        return ResponseEntity.ok(baseService.save(lotGiayDto));
    }

    @PutMapping("/{id}")
    @Operation(summary = "update theo id")
    public ResponseEntity<LotGiayDto> updateLotGiay(@PathVariable int id, @RequestBody LotGiayDto lotGiayDto) {
